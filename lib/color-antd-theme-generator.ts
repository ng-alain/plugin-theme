...
/*
  ...
  @link-color: #1890ff
*/
function getColor(varName: string, mappings: { [key: string]: any }): any {
...
*/
function generateColorMap(content: string, customColorRegexArray = []) {
  return content
    .split('\n')
...
function getMatches(string: string, regex: RegExp): any {
...
*/
function render(text: string, paths: string[]) {
...
*/
function getLessVars(filtPath: string) {
  const sheet = fs.readFileSync(filtPath).toString();
  const lessVars: { [key: string]: any } = {};
...
*/
function isValidColor(color: string, customColorRegexArray: RegExp[] = []) {
...
async function compileAllLessFilesToCss(stylesDir: string, antdStylesDir: string, varMap: { [key: string]: any } = {}, varPath: string) {
  /*
    Get all less files path in styles directory
    and then compile all to css and join
  */
  const styles: string[] = glob.sync(path.join(stylesDir, './**/*.less'));
  const csss = await Promise.all(
    styles.map(filePath => {
      let fileContent = fs.readFileSync(filePath).toString() as string;
      ...
      fileContent = fileContent.replace(r, (match, importPath, index, content) => {
...
export async function generateTheme({
  ...
  try {
    ...
    */
    ...
    let content = '';
    styles.forEach((filePath: string) => {
    ...
    hashCache = hashCode;
    let themeCompiledVars: any = {};
    let themeVars: string[] = themeVariables || ['@primary-color'];
    const lessPaths = [path.join(antdPath, './style'), stylesDir];

    const randomColors: any = {};
    const randomColorsVars: any = {};
    /*
  ...
  */
    ...
    let css = '';
    themeVars = themeVars.filter((name: any) => name in mappings && !name.match(/(.*)-(\d)/));
...
function combineLess(filePath: string, nodeModulesPath: string) {
